...
/* September 2013
Modified by Peter Holak for the Rail Climber project - OffsetPolygons now also returns
a map specifying which of the original vertices were squared when using jtMiter */
...
private:
  ...
  std::map<int, bool> *m_squared;
 
public:

OffsetBuilder(const Polygons& in_polys, Polygons& out_polys,
  bool isPolygon, double delta, JoinType jointype, EndType endtype, double limit, std::map<int, bool> *squared = 0): m_p(in_polys), m_squared(squared)
{
...
void OffsetPolygons(const Polygons &in_polys, Polygons &out_polys,
  double delta, JoinType jointype, double limit, bool autoFix, std::map<int, bool> *mitered)
{
  if (!autoFix && &in_polys != &out_polys)
  {
    OffsetBuilder(in_polys, out_polys, true, delta, jointype, etClosed, limit, mitered);
    return;
  }
  ...
  OffsetBuilder(inPolys, out_polys, true, delta, jointype, etClosed, limit, mitered);
}
//------------------------------------------------------------------------------
