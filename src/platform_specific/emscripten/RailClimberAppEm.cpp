...
void RailClimberAppEm::initOpenGl() {
...
void RailClimberAppEm::cleanupOpenGl() {
    SDL_Quit();
}

RailClimberAppEm::RailClimberApp(const string &settingsFile) : mSdlScreen(nullptr), mPlayStatusRegistry(mMetadataRegistry) {
    ...
    Log::info("Got here, %d x %d", mScreenWidth, mScreenHeight);
    ...
    auto unitConverter = make_shared<UnitConverter>(mScreenWidth, mScreenHeight, mGameWidth, mGameHeight);
    ...
    mAssetRegistry = new SharedAssetRegistry();
    mRenderDriver = new RenderDriver(mAssetRegistry);
    mGameScene = new GameScene(unitConverter, mAssetRegistry, mScreenWidth, mScreenHeight, mGameWidth, mGameHeight, mMetadataRegistry, mPlayStatusRegistry, mRenderDriver);
    // TODO: ...
    ...
    Painter *painter = new Painter(unitConverter);
    painter->initialize(mScreenWidth, mScreenHeight, 1.0f);
    painter->createGraphics();

    mGameMenu = new GameMenu(painter, mMetadataRegistry, mPlayStatusRegistry, mGameScene);
    mGameMenu->registerWithSceneManager(mSceneManager);

    mSceneManager->pushScene(mGameMenu->mainMenu);

    if (MULTIPLE_THREADS) {
        pthread_create(&mRenderThread, nullptr, RailClimberAppEm::threadStartHelper, this);
    }
...
RailClimberAppEm::~RailClimberApp() {

}

int RailClimberAppEm::run() {
    emscripten_set_main_loop_arg(RailClimberAppEm::emscriptenLoopHelper, this, -1, true);
...
void RailClimberAppEm::emscriptenLoop() {
    handleEvents();
    tick();
...
void RailClimberAppEm::handleEvents() {
    SDL_Event event;
...
void RailClimberAppEm::handleKeyboardEvent(const SDL_Event &event) {
    ...
    switch (event.key.keysym.sym) {
        ...
        case SDLK_UP:
            ...
            //speed *= 2;
            //mainTimer = SDL_AddTimer(speed, timerProc, NULL);
            break;
...
void RailClimberAppEm::handleMouseEvent(const SDL_Event &event) {
...
void RailClimberAppEm::moveCursorToPlayer() {
    World *world = mGameScene->getWorld();

    const World::Player& player = world->getPlayer();
    float xShift = 0, yShift = 0;
/*
    if (mPainter->scrollX > 0 && mPainter->scrollX - (world->levelW - mGameWidth) < -0.01) {
        xShift = -player.pos.x() + (mGameWidth / 2);
    } else if (mPainter->scrollX > 0) {
        xShift = -world->levelW + mGameWidth;
    }
...
void RailClimberAppEm::reloadOpenGl() {
...
void RailClimberAppEm::runRenderThread() {
